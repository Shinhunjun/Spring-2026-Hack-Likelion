...
> **프로젝트 종료 시까지 아래 항목 중 사유 없이 2회 이상 위반 시, 최종 프로젝트 결과물(Abstract, 논문 등)에서 이름이 제외됩니다.**

| 위반 항목 | 설명 |
|----------|------|
| **Commit 미제출** | 주차별 마감 기한까지 본인 폴더에 commit/push를 하지 않은 경우 |
| **모임 무단 불참** | 사전 연락 없이 주간 미팅에 참석하지 않은 경우 |

- 사유가 있는 경우(질병, 시험 등) **사전에** 팀에 알리면 위반으로 간주하지 않습니다.
- 위반 횟수는 GitHub commit 기록과 미팅 출석으로 투명하게 관리됩니다.
